...
func TestCycle(t *testing.T) {
	...
	*(*uintptr)(unsafe.Pointer(&heapObjects[1])) = uintptr(unsafe.Pointer(&heapObjects[2]))
	*(*uintptr)(unsafe.Pointer(&heapObjects[2])) = uintptr(unsafe.Pointer(&heapObjects[1]))

	var stacks = [][]uintptr{
		{
			uintptr(unsafe.Pointer(&heapPointer1)), uintptr(unsafe.Pointer(&heapPointer2)), 0x00,
		},
	}

...
func TestCOWBufferExtra(t *testing.T) {
	data := []byte{'a', 'b', 'c', 'd'}
	buffer := NewCOWBuffer(data)
	defer buffer.Close()
	...
	assert.Equal(t, unsafe.SliceData(copy1.data), unsafe.SliceData(buffer.data))
	assert.Equal(t, unsafe.SliceData(copy2.data), unsafe.SliceData(buffer.data))
	assert.NotEqual(t, unsafe.SliceData(copy3.data), unsafe.SliceData(buffer.data))

	assert.True(t, reflect.DeepEqual(3, *copy1.refs))
	assert.True(t, reflect.DeepEqual(3, *copy2.refs))
	assert.True(t, reflect.DeepEqual(1, *copy3.refs))
}
